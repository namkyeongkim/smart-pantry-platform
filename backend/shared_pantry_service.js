...
module.exports = {
  ...
  createPantry: async (userId, name) => {
    ...
    // Create a virtual user in Postgres to store shared pantry items.
  ...
  joinPantry: (userId, code) => {
    const state = loadState();
    const pantry = state.pantries.find(
  ...
  // Return all virtual user IDs used for shared pantry storage.
  getSharedUserIds: () => {
    const state = loadState();

    return state.pantries
      .map((pantry) => pantry.virtualId)
      .filter(Boolean);
  },

  // Return all shared pantries for admin analytics.
  getAllSharedPantries: () => {
    const state = loadState();
    return state.pantries;
  },
};